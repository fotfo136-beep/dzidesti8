...
export function numberToWords(num: number): string {
  ...
  const Ghana Cedis = convertThousands(Math.floor(num));
  constGp = Math.round((num - Math.floor(num)) * 100);
  const pesewas = convertHundreds(pesewas);

  return `${Ghana Cedis} Ghana Cedis${pesewas ? ` and ${pesewas} Pesewas` : " Only"}`;
}
